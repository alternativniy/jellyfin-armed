#!/usr/bin/env bash
set -euo pipefail

sonarr_ensure_dirs() {
  mkdir -p "$CONFIG_PATH/configs/sonarr"
  mkdir -p "$MEDIA_PATH/tv"
}

sonarr_wait_ready() {
  wait_for_http "sonarr" "http://127.0.0.1:8989" || wait_for_tcp "sonarr" 127.0.0.1 8989 || true
}

sonarr_find_api_key() {
  local cfg="$CONFIG_PATH/configs/sonarr/config.xml"
  [[ -f "$cfg" ]] || return 0
  grep -oP '(?<=<ApiKey>)[^<]+' "$cfg" | head -n1 || true
}

sonarr_configure() {
  local k="${1-}"; [[ -z "$k" ]] && log "[sonarr] missing API key" && return 0
  local base="http://127.0.0.1:8989/api/v3"; local header=( -H "X-Api-Key: $k" )
  local existing=$(http_get_json "$base/rootfolder" "${header[@]}" | grep -oE '"path":"[^"]+"' || true)
  if ! grep -q '/tv' <<<"$existing" && ! grep -q '/media/tv' <<<"$existing"; then
    log "[sonarr] adding root folder /tv"; http_post_json "$base/rootfolder" '{"path":"/tv","accessible":true}' "${header[@]}" >/dev/null 2>&1 || true
  fi

  # Ensure qBittorrent credentials (use stored file or prompt if interactive)
  if [[ -z "${QB_USERNAME:-}" ]]; then prompt_var QB_USERNAME "qBittorrent WebUI username" "admin"; fi
  if [[ -z "${QB_PASSWORD:-}" ]]; then
    if [[ -f "${JARM_DIR:-$HOME/.jarm}/qbittorrent_password.txt" ]]; then
      QB_PASSWORD="$(cat "${JARM_DIR:-$HOME/.jarm}/qbittorrent_password.txt" 2>/dev/null)"; export QB_PASSWORD
    else
      prompt_var QB_PASSWORD "qBittorrent WebUI password" "adminadmin"
    fi
  fi
  if [[ -n "${QB_USERNAME:-}" && -n "${QB_PASSWORD:-}" ]]; then
    local clients
    clients=$(curl -sS -H "X-Api-Key: $k" "$base/downloadclient" || true)
    if ! echo "$clients" | grep -qi qbittorrent; then
      local client_json
      client_json=$(cat <<EOF
{
  "name": "qBittorrent",
  "protocol": "torrent",
  "implementation": "QBittorrent",
  "configContract": "QBittorrentSettings",
  "fields": [
    {"name":"host","value":"qbittorrent"},
    {"name":"port","value":8080},
    {"name":"useSsl","value":false},
    {"name":"urlBase","value":""},
    {"name":"username","value":"${QB_USERNAME}"},
    {"name":"password","value":"${QB_PASSWORD}"},
    {"name":"tvCategory","value":"tv"},
    {"name":"recentPriority","value":0},
    {"name":"olderPriority","value":0},
    {"name":"sequentialOrder","value":true},
    {"name":"removeCompleted","value":true},
    {"name":"addPaused","value":false},
    {"name":"useCategory","value":true},
    {"name":"enable","value":true}
  ],
  "enable": true,
  "removeCompleted": true,
  "id": 0
}
EOF
)
      curl -sS -H "X-Api-Key: $k" -H "Content-Type: application/json" -d "$client_json" "$base/downloadclient" >/dev/null 2>&1 || true
      log "[sonarr] added qBittorrent download client"
    else
      log "[sonarr] qBittorrent client already present"
    fi
  else
    log "[sonarr] qBittorrent credentials missing; skip download client add"
  fi
}
